...
public class MoonTimedQueueConfig extends MoonConfig {
	...
	public static String DEFALUT_CONFIG_PROPERTIES_FILE = "config.properties";
	...
	public void validation() {
		super.validation();
		if (processLimit < 0) {
			logger.warn(KEY_PROCESS_LIMIT + " value should be greater than 0, use default value [" + DEFAULT_PROCESS_LIMIT + "] instead.");
			this.processLimit = 0;
		}
		if (initBufferSize <= 0) {
			logger.warn(KEY_INIT_BUFFER_SIZE + " value should be greater equal than 0, use default value [" + DEFAULT_INIT_BUFFER_SIZE + "] instead.");
		}
		if (processWaitTimes < 0) {
			logger.warn(KEY_PROCESS_WAIT_TIMES + " value should be greater than 0, use default value [" + DEFAULT_PROCESS_WAIT_TIMES + "] instead.");
		}
	}
